...
import {
  ...
  FileText, LogOut, Zap, Calendar, Settings, Cpu, Layers
} from 'lucide-react';
...
export default function NavBar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="p-6 border-b border-[rgba(255,79,216,0.1)]">
      ...
      <nav className="flex-1 p-4 space-y-1">
        {NAV_ITEMS.map(({ to, icon: Icon, label, id }) => (
          ...
          >
            {({ isActive }) => (
              <>
                <Icon size={18} style={{ color: isActive ? '#ff4fd8' : undefined }} />
                {label}
                {isActive && (
                  <motion.div
      ...
      {/* System Status */}
      <div className="p-4 mx-4 mb-3 rounded-xl" style={{ background: 'rgba(255,79,216,0.04)', border: '1px solid rgba(255,79,216,0.08)' }}>
      ...
      {/* User + Logout */}
      <div className="p-4 border-t border-[rgba(255,79,216,0.08)]">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-[#64748b] hover:text-[#ff3d3d] hover:bg-[rgba(255,61,61,0.08)] transition-all duration-200"
        >
          <LogOut size={14} />
          Sign Out
        </button>
      ...
      {/* Footer Clock */}
      <div className="p-4 border-t border-[rgba(255,79,216,0.05)] flex items-center gap-2 text-[10px] text-[#475569] justify-center bg-[rgba(0,0,0,0.15)]">
